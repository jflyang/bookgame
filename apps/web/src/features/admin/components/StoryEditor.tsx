...
const nodeGuides: Record<WorkflowNodeId, {
  ...
  performances: {
    title: "演出配置",
    what: [
      "演出 = 触发器 + 渲染器。LLM 生成回复后，客户端检查最新消息是否命中某个 trigger，命中则播放对应的音频/图片/视频。",
      "",
      "五种触发器类型：",
      "① 知识库命中（knowledgeUse）—— 消息内容匹配知识库标题或触发词。最常用，支持粗体精确匹配和全文模糊匹配。",
      "② 技能使用（skillUse）—— LLM 在 action 中使用了某个技能（action.type = \"skill\"）。需先在角色配置中定义技能。",
      "③ 消息事件（messageEvent）—— 消息携带了指定的事件 ID。用于服务端注入的自定义事件，需代码配合。",
      "④ 首次出场（firstAppearance）—— 某个角色在当前会话中第一次发言。适合登场音效。",
      "⑤ 阶段进入（stageEnter）—— 剧情推进到某个阶段时触发。不依赖消息内容。",
    ].join("\n"),
    impact: [
      "写入 manifest.json → performances 字典。玩家端 StoryPerformanceRuntime 每收到新消息都会遍历所有演出检查触发条件。",
      "",
      "技能→演出联动数据流：",
      "1. 技能定义在 skills.json 或角色知识库（Markdown）中",
      "2. LLM 输出 action.type = \"skill\" + action.skillId",
      "3. turnProcessor 把 skillId 写入 message.usedSkills",
      "4. StoryPerformanceRuntime 检查 message.usedSkills 是否包含 trigger.skillId",
      "5. 命中 → 播放演出",
      "",
      "知识库→演出联动数据流：",
      "1. 知识库文档中写入 表演: {名称} 和 触发热：xxx",
      "2. LLM 输出包含匹配关键词的文本",
...
function runtimeSteps(id: WorkflowNodeId) {
  ...
  const focus: Record<WorkflowNodeId, string[]> = {
    ...
    performances: ["LLM 生成", "状态结算", "玩家界面展示"],
